...
export default function Home() {
 ...
 return <main className="shell">
  <header className="topbar"><a className="brand" href="/"><BookOpen size={25}/>agent<span>book</span><b>•</b></a><span className="edition">THE READER’S COMPANION</span><Select value={mode} onValueChange={value=>{setMode(value);setBudget('all');void load(query,tab,value);}}><SelectTrigger aria-label="데이터 모드" className="mode"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="demo">예시 데이터</SelectItem><SelectItem value="live">실제 도서</SelectItem></SelectContent></Select></header>
  <section className="intro"><p className="eyebrow">READ MORE. CHOOSE BETTER.</p><h1>다음 책의 발견,<br/><span>가격부터 취향까지.</span></h1><p>읽고 싶은 책을 찾고, 나에게 맞는 가격을 비교하세요.</p></section>
  <form className="searchbox" onSubmit={e=>{e.preventDefault();void search(query);}}><Search aria-hidden="true"/><input aria-label="책 검색" placeholder="책 제목, 저자 또는 ISBN을 입력하세요" maxLength={100} value={query} onChange={e=>setQuery(e.target.value)}/><button type="submit" disabled={busy}>{busy?'찾는 중':'책 찾기'} <ArrowUpRight size={18}/></button></form>
  <div className="search-meta"><div className="suggestions"><span>검색해 보세요</span>{(mode==='demo'?['소설','습관','과학']:['한강','인공지능','한국소설']).map(q=><button key={q} onClick={()=>void search(q)}>{q}</button>)}</div><span className="connection">알라딘 {status.aladin?'키 설정됨':'미연결'} · 네이버 {status.naver?'키 설정됨':'미연결'}</span></div>
  ...
  <div className="notice"><Info size={17}/><span>{mode==='demo'?'체험용 예시입니다. 도서·서점·가격·순위는 모두 가상 데이터이며, 실제 도서는 상단에서 전환할 수 있습니다.':'동일 ISBN의 새 종이책 정보를 모읍니다. 배송비·쿠폰·적립금은 서점에서 확인하세요.'}</span></div>
  <Tabs value={tab} onValueChange={value=>{setTab(value);setBudget('all');void load(value==='trends'?'':query,value,mode);}}>
   <TabsList className="main-tabs" variant="line"><TabsTrigger value="discover"><BookOpen/>도서 탐색</TabsTrigger><TabsTrigger value="trends"><TrendingUp/>독서 트렌드</TabsTrigger></TabsList>
   ...
   <TabsContent value="discover">
    ...
    {busy?<div className="book-grid">{[1,2,3].map(i=><Skeleton key={i} className="h-72 rounded-xl"/>)}</div>:visible.length?<div className="book-grid">{visible.map((book,i)=><article className="book-card" key={book.id}><div className="card-top"><span className="category">{book.category}</span><span className="index">{String(i+1).padStart(2,'0')}</span></div>{book.cover&&<img className="cover" src={safeUrl(book.cover)} alt={`${book.title} 표지`} loading="lazy" onError={e=>{e.currentTarget.style.display='none';}}/>}<button className="book-title" onClick={()=>setDetail(book)}><h3>{book.title}</h3></button><p>{book.author} · {book.publisher}</p><Price book={book} demo={catalog.demo}/><button className="detail" onClick={()=>setDetail(book)}>가격 자세히 보기 <ArrowUpRight size={17}/></button><label className="compare-check"><Checkbox checked={compared.some(b=>b.id===book.id)} disabled={compared.length>=3&&!compared.some(b=>b.id===book.id)} onCheckedChange={v=>toggleCompare(book,v===true)}/>비교함에 담기</label></article>)}</div>:<Empty className="empty-panel"><Search size={28}/><EmptyTitle>{mode==='live'&&!submitted?'어떤 책을 찾고 있나요?':'조건에 맞는 책이 없습니다'}</EmptyTitle><EmptyDescription>제목·저자·ISBN으로 검색하거나 필터를 초기화해 보세요.</EmptyDescription><button className="text-button" onClick={reset}>검색 조건 초기화</button></Empty>}
